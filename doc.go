...
//
// 注意：**服务端对所有距离都照记不误，不会拒签**（这一条由项目所有者、
// 本校学生确认，不是我从抓包测出来的）。所以 2000m 只是教师端的展示阈值，
// 它不阻止签到成功，衹是让记录在教师端看起来是「异常」——也就是说
// 伪造坐标不会导致签到失败，但可能换来一个被标记的考勤记录。
//
// ## 信息在客户端就被丢掉了
//
...
//
// ## 6. 定位由客户端上报，可伪造；但服务端确实会算距离
//
// 详见上面「定位信息」一节。要点：后端只拿到两个 float
// （客户端把 `accuracy`、`isFromMock` 等字段全部丢掉了），所以从后端视角
// 定位是纯客户端输入、可伪造；服务端会用**教师生成签到码时的定位**算出
// distance，教师端把 >= 2000m 的标为「异常」。
//
// **服务端不因距离超限拒签**（由项目所有者确认），所以伪造坐标不会导致
// 签到失败，但可能换来一条被教师端标记为「异常」的记录。
// 缺失定位会导致签到失败。
//
// ## 7. 会话 token 的生命周期未知
//

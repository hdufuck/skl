...
**签到方式 Method**:
`pkg/signin` 里一条可枚举的签到路径（`analyze-a0` / `code-check-in` / `captcha-verify-missing` / `captcha-verify-forged` / `captcha-verify-genuine`）。这些是**给调用方的库 API**；探针阶梯只保留 `captcha-verify-genuine` 一档，其余四条仅可显式调用（见 [ADR 0004](docs/adr/0004-阶梯只保留真值档.md)）。

...
> 本仓库**按「强制」前提运作、不再探测**（缺参只有单边结果，而缺参档本身就是一次
> 未鉴权的写入尝试），决策见 [ADR 0004](./adr/0004-阶梯只保留真值档.md)。
...
> 唯一需要用**真实有效签到码**跑的实验是「库的 `SignIn` 封装路径可用」：
> 方法、预置与脱敏规则见 **[签到探针手册](./signin-probe.md)**（只能做一次）。

| 项 | 状态 |
| --- | --- |
| `captcha-verify` 成功响应结构 | ✅ 已实测（`har#3` 浏览器抓包；全文见 [`signin-success-sample.md`](./signin-success-sample.md)） |
| `captcha-verify` 是否**强制**人机验证 | **按强制前提运作，不再探测**（[ADR 0004](./adr/0004-阶梯只保留真值档.md)）。注意这**不是已实测**：已知「有效签到码 + 人机不通过」= `200 F001`，但始终**没有**「有效签到码 + 缺失参数」的样本（见 3.2），缺参只能拿到不可解释的 `401` |
...
要验证**库的 `SignIn` 封装路径可用**时，按 **[签到探针手册](./signin-probe.md)** 执行：
抓包环境、单变量纪律、真值档请求后的考勤记录读回与脱敏规则都写在那里，
不要临场发挥——那个窗口不可重复。（「人机是否强制」不在探针要回答的问题里，
见 [ADR 0004](./adr/0004-阶梯只保留真值档.md)。）

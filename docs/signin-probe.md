...
5. **Reqable 上报服务器**（手机端操作见 4.3）：
   手机 Reqable → `⋮` → `更多` → `上报服务器`，新建规则：
   - URL 匹配：`*captcha-verify*`
   - 接收地址：脚本启动时打印的 `http://<Mac局域网IP>:8080/hook`
...
1. 手机装 Reqable（`com.reqable.android`），开启抓包（独立 VPN 模式即可），
   并确认证书已装好、能解密 HTTPS。
2. 登录一个免费 Reqable 账号。
3. `⋮` → `更多` → `上报服务器` → 新建：
   - 名称：`skl-captcha`
   - URL 匹配：`*captcha-verify*`
   - 服务端地址：`http://<脚本打印的Mac局域网IP>:8080/hook`
   - 压缩：`gzip`（脚本会自动解，HAR 仍是 JSON）
4. 在手机钉钉里打开 skl 签到页，保持登录态；确认 Mac 与手机在同一局域网。
...
4. 看到签到成功后**不用管脚本**——Reqable 会自动把那条 `captcha-verify`
   请求/响应 HAR POST 到脚本，脚本把它并入报告。

没收到上报时：检查 URL 匹配规则、Mac 局域网 IP、手机与 Mac 是否同网段；
手机抓包本身仍在 Reqable 里，可事后人工抄录（只抄响应体即可）。
